...
- TTS 朗读
  - 支持浏览器系统语音、MiniMax HTTP TTS 和 OpenAI 兼容 TTS
  - 远程 TTS 生成后自动保存到浏览器 IndexedDB，之后播放同一段内容时直接使用缓存
  - 短按消息正文/引号语音按钮播放，长按约 650ms 强制重新生成并覆盖缓存
  - 消息操作菜单中的下载按钮可将正文语音保存为本地音频文件
- 启动更新提醒
  - 进入 SillyTavern 后自动检查 Git 仓库是否有新提交
  - 发现更新时弹窗确认，用户同意后调用 SillyTavern 扩展更新接口并刷新页面
...
若要使用启动更新提醒，请在 SillyTavern 的“安装扩展”中填写本项目 Git 仓库 URL 进行安装。手工复制或从 ZIP 解压的目录不包含 Git 信息，SillyTavern 无法对其执行自动更新。
...
使用 MiniMax 或 OpenAI 兼容 TTS 时，第一次播放会请求接口，并把返回的音频保存在当前浏览器中。后续即使刷新页面，只要正文和模型、音色、语速等配置没有变化，短按语音按钮就会直接播放缓存，不再请求接口。

- 短按喇叭：播放正文语音，优先读取浏览器缓存
- 长按喇叭：忽略旧缓存，重新生成并覆盖保存
- 引号按钮：对双引号内容执行相同的短按/长按逻辑
- 下载按钮：优先下载缓存；没有缓存时会先生成并保存，再下载
- 暂停/继续按钮：暂停当前语音，再次点击可从暂停位置继续
- 停止按钮：停止当前朗读

插件设置中的“已生成语音”列表可以查看浏览器缓存，支持单条播放、下载、删除，也可以勾选多条后打包下载为 ZIP。

浏览器系统语音由设备实时朗读，浏览器不会提供音频文件，因此该模式无法缓存或下载。
...
- TTS 缓存位于当前浏览器的 IndexedDB 中，不会自动写入 SillyTavern 服务端；清理站点数据会同时删除缓存。
